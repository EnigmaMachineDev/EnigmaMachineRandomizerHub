document.addEventListener('DOMContentLoaded', () => {
    ...
    function initializeOptions(data) {
        // Weapons
        if (data.weapons) {
            const weaponsList = Object.keys(data.weapons).map(key => ({ name: key }));
            addCategory('weapons', 'Weapons', weaponsList);
        }
        
        // Armor - combine all types
        if (data.armor) {
            const allArmor = [
                ...(data.armor.light || []),
                ...(data.armor.medium || []),
            ...
            addCategory('armor', 'Armor', allArmor);
        }
        
        // Spirit Ashes
        if (data.spirit_ashes) {
            addCategory('spirit_ashes', 'Spirit Ashes', data.spirit_ashes);
        }
        
        // Spells
        if (data.spells) {
            addCategory('spells', 'Spells', data.spells);
        }
        
        // Casting Weapons - Seals
        if (data.casting_weapons?.seals) {
            addCategory('seals', 'Seals', data.casting_weapons.seals);
        }
        
        // Casting Weapons - Staffs
        if (data.casting_weapons?.staffs) {
            addCategory('staffs', 'Staffs', data.casting_weapons.staffs);
        }
        
        // Endings
        if (data.endings) {
            addCategory('endings', 'Endings', data.endings);
        }
    }
